...
def goal_form_guide(game_week):
	...
	match = 5
	while i <= game_week:
		df_goals.loc[df_goals["WK "+str(i)] > 0, "WK-"+str(match)] = "⚽"
		df_goals.loc[df_goals["WK "+str(i)].isnull(), "WK-"+str(match)] = "➖"
		i = i + 1
		match = match - 1
	return
goal_form_guide(game_week)

df_goals["FORM"] = df_goals["WK-1"]+df_goals["WK-2"]+df_goals["WK-3"]+df_goals["WK-4"]+df_goals["WK-5"]
...
# ---- BOARDROOM DF BUILD
df_broom = pd.read_excel(excel_file_season, skiprows=7, nrows=21, sheet_name='Board Room', usecols=[12,13]).fillna(0)
...
if menu_selection == "Board Room":
	st.dataframe(df_broom, width=None, height=775, use_container_width=True, hide_index=True, column_config={"POSITION": " ","Unnamed: 12": "PLAYER", "Unnamed: 13": "VISITS"})
