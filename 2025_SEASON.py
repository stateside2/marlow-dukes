...
game_week = 48
File_Date = "2025 FINAL"

excel_file = "data/2025.xlsx"
excel_file_prev: str = "data/2025_prev.xlsx"
...
# formguide_stat_cols=[0,52,54,55,56,57,(game_week-4),(game_week-3),(game_week-2),(game_week-1),game_week] #--- THIS WILL BREAK WHEN GAME_WEEK < 5
formguide_stat_cols=[0,50,51,55,56,(game_week-4),(game_week-3),(game_week-2),(game_week-1),game_week] #--- THIS WILL BREAK WHEN GAME_WEEK < 5
df_full_tab = pd.read_excel(excel_file, skiprows=[0,1,3], nrows=37, sheet_name='League Table', usecols=formguide_stat_cols)
...
df_tab_prev = pd.read_excel(excel_file_prev, skiprows=[0,1,3], nrows=37, sheet_name='League Table', usecols=[0,50])
...
if stat_selection == "Goals":
	...
	df_goals = pd.read_excel(excel_file, skiprows=[0,1,3], nrows=23, sheet_name='Goals', usecols=[0,52])
	df_goals = df_goals.sort_values(by=["TOTAL", "PLAYER"], ascending=[False, True])
	df_goals.insert(0, "POSITION", range(1, 1 + len(df_goals)))

	st.dataframe(df_goals, width=None, height=843, use_container_width=True, hide_index=True, column_config={"POSITION": " ","TOTAL": "GOALS"})

# -------------

# ---- MOTM DF BUILD
if stat_selection == "MOTM":
	df_motm = pd.read_excel(excel_file, skiprows=[1,35,36,37,38,39,40,41,42], sheet_name='MOTM', usecols=[0,52])
	df_motm = df_motm.sort_values(by=["VOTES", "PLAYER"], ascending=[False, True])
	df_motm.insert(0, "POSITION", range(1, 1 + len(df_motm)))

	st.dataframe(df_motm, width=None, height=1192, use_container_width=True, hide_index=True, column_config={"POSITION": " "})
...
if stat_selection == "Board Room":
	...
	st.dataframe(df_broom, width=None, height=808, use_container_width=True, hide_index=True, column_config={"POSITION": " ","Unnamed: 12": "PLAYER", "Unnamed: 13": "VISITS"})

# -------------

# --- APEARANCES. USED IN RATIO CALCULATIONS.
df_appear = pd.read_excel(excel_file, skiprows=[0,1,3], nrows=37, sheet_name="League Table", usecols=[0,51])
df_appear = df_appear.sort_values(by=["PLAYED", "PLAYER"], ascending=[False, True])

# --- WIN RATIO
if stat_selection == "Win Ratio":
	df_win_ratio = pd.read_excel(excel_file, skiprows=[0,1,3], nrows=37, sheet_name="League Table", usecols=[0,51,52])
...
if stat_selection == "Loss Ratio":
	df_loss_ratio = pd.read_excel(excel_file, skiprows=[0,1,3], nrows=37, sheet_name="League Table", usecols=[0,51,54])
...
if stat_selection == "Points per match":
	df_pts_match = pd.read_excel(excel_file, skiprows=[0,1,3], nrows=37, sheet_name="League Table", usecols=[0,51,55])
...
if stat_selection == "MOTM votes per match":
	df_motm = pd.read_excel(excel_file, skiprows=[1,38,39,40,41,42], sheet_name='MOTM', usecols=[0,52])
